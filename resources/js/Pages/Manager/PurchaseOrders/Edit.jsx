...
export default function Edit({ purchaseOrder, outlets, suppliers, products }) {
    const [selectedOutlet, setSelectedOutlet] = useState({
        value: purchaseOrder.outlet_id,
        label: purchaseOrder.outlet?.name
    });
    const [selectedSupplier, setSelectedSupplier] = useState({
        value: purchaseOrder.supplier_id,
        label: `${purchaseOrder.supplier?.name} - ${purchaseOrder.supplier?.company}`
    });
    ...
    const submit = (e) => {
        ...
        const formData = {
            ...
            items: items.map(item => ({
                id: item.id > 1000000000000 ? null : item.id, // New items have timestamp IDs
                product_id: item.product_id,
                quantity: item.quantity,
                unit_price: item.unit_price,
                total_price: item.total_price
        ...
        put(route('manager.purchase-orders.update', purchaseOrder.id), {
            data: formData
        });
    };
                            ...
                            {purchaseOrder.status !== 'draft' && (
                                <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
                                    <div className="text-yellow-800">
                                        <strong>Note:</strong> This purchase order has status "{purchaseOrder.status}".
                                ...
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            className="mt-1"
                                            isSearchable
                                            isDisabled={purchaseOrder.status !== 'draft'}
                                        />
