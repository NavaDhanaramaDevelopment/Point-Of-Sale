...
export default function CustomerDebts({ auth, debts, filters, customers = [], sales = [], outlets = [] }) {
    ...
    const handleCreateDebt = (e) => {
        e.preventDefault();
        post(route('customer-debts.store'), {
            onSuccess: () => {
                reset();
    ...
    const submitPayment = (e) => {
        e.preventDefault();
        post(route('customer-debts.payment', selectedDebt.id), {
            data: {
                amount: data.amount,
    ...
    const deleteDebt = (debt) => {
        if (confirm('Are you sure you want to delete this debt?')) {
            router.delete(route('customer-debts.destroy', debt.id));
        }
    };
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-gray-900">
                                                        ...
                                                        <div className="text-red-600 font-medium">
                                                            Remaining: Rp {parseFloat(debt.remaining_amount).toLocaleString()}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
